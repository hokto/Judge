...
int main(){
	...
	hoge[st.F][st.S]=1;
	for(int i=st.F;i<h;i++){
		if(table[i][st.S]=='#')break;
		hoge[i][st.S]=1;
	}
	for(int i=st.S;i<w;i++){
		if(table[st.F][i]=='#')break;
		hoge[st.F][i]=1;
	}
	arep(i,st.F+1,h)arep(j,st.S+1,w){
		if(table[i][j]=='#')continue;
		hoge[i][j]=(hoge[i-1][j]+hoge[i][j-1])%MOD;
	}
	if(hoge[go.F][go.S]<=0)out(-1);
	else out(hoge[go.F][go.S]);
	return 0;
}
